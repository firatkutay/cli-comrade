...
- **`comrade upgrade` could desync a Node package manager's own installed version.** A Node-package-manager-managed install (`npm/main/bin/comrade.js` spawning the real binary out of `node_modules` — npm, pnpm, yarn, and bun all resolve identically) self-updating in place would leave the package manager's own bookkeeping reporting the old version, and its next update command would silently revert the binary. `comrade upgrade` now refuses when it detects this — via `COMRADE_MANAGED_BY=npm`, set by the dispatcher in the child env (primary signal), or a `node_modules` path segment in the resolved running executable (fallback, for a direct invocation that bypasses the dispatcher) — and points at using that package manager instead, npm given as the worked example. `--check` is deliberately never refused (it downloads and mutates nothing) — it still reports the result and appends the same remediation. The passive "a new version is available" notice is likewise never suppressed; it points at the package manager instead of `comrade upgrade`.
- **`comrade doctor` used to advise `comrade upgrade` even when it would refuse.** `VersionCheck`'s `Fix` field now agrees with `comrade upgrade`'s own refusal under a Node-managed install instead of recommending a command that immediately fails.
- **`COMRADE_MANAGED_BY` no longer leaks into spawned commands.** `internal/executor` now strips it from every child process's environment, so a plan step that itself invokes another comrade binary can no longer inherit a false "don't self-update" signal from an unrelated parent process — and the variable, being user-settable, can no longer be used to pin comrade against ever accepting a security update.
- Docs (`TROUBLESHOOTING.md`, `GUIDE.md`, `npm/main/README.md`) updated to describe the refusal and its `--check` carve-out instead of contradicting it.
...
- **winget manifest PR now opens correctly** — the previous release used a token that could not create a pull request to microsoft/winget-pkgs. Homebrew and Scoop, live since 0.1.2, re-publish here to stay in sync.
...
[Unreleased]: https://github.com/firatkutay/cli-comrade/compare/v0.4.3...HEAD
