...
- Bulgu (bağımsız review): ask-modu onay satırı (internal/tui/confirm.go:189 + mapKey) `[e]vet [h]ayır [d]üzenle [a]çıkla [t]ümü` olarak sabit Türkçe yazılı ve `general.language`'ı takip etmiyor — İngilizce arayüzde de bu Türkçe metin gösteriliyor (EN dokümantasyonu daha önce olmayan bir İngilizce tuş kümesi varsaymıştı, bu da düzeltildi). Takip işi: internal/i18n üzerinden, dile göre kabul tuşları (ör. EN için `[y]es [n]o [e]dit [x]plain [a]ll`) ile yönlendirilmeli.
